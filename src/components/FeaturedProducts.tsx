          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-10">
            {products.map((product, index) => (
              <motion.div
                ...
                className="group relative aspect-square md:aspect-[4/3] lg:aspect-[1.1/1] rounded-[1.5rem] md:rounded-[3rem] overflow-hidden cursor-pointer shadow-[0_32px_64px_-16px_rgba(0,0,0,0.4)] w-full max-w-lg mx-auto md:max-w-none"
              ...
              >
                ...
                {product.discount && (
                  <span className="absolute top-4 right-4 md:top-6 md:right-6 z-10 bg-primary text-black text-[9px] md:text-xs font-bold px-3 py-1.5 md:px-4 md:py-2 rounded-full shadow-2xl backdrop-blur-md">
                    {product.discount}% OFF
                  </span>
                )}

                {/* Content Overlay (Bottom Left & Right) */}
                <div className="absolute inset-x-0 bottom-0 p-6 md:p-12 flex items-end justify-between gap-4 md:gap-6 z-20">
                  <div className="space-y-1 md:space-y-2">
                    <h3 className="font-display text-xl md:text-4xl text-white font-bold tracking-widest uppercase leading-tight">
                      {product.name}
                    </h3>
                    <div className="flex items-baseline gap-3 md:gap-4">
                      ...
                      {product.originalPrice && (
                        <span className="text-white/40 line-through text-[10px] md:text-sm font-light font-numbers-inter">
                  ...
                  <button
                    ...
                    className="w-10 h-10 md:w-16 md:h-16 rounded-full bg-primary text-black flex items-center justify-center shadow-2xl hover:scale-110 transition-all duration-500 active:scale-95 shrink-0 group/btn"
                    aria-label="Add to cart"
                  >
                    <ShoppingBag className="w-5 h-5 md:w-8 md:h-8 group-hover/btn:rotate-12 transition-transform" />
                  </button>
                </div>
              </motion.div>
            ))}
